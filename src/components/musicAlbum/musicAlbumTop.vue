<template>
  ...
  <div v-if="Object.keys(playlist).length > 0">
    <div class="blur"></div>
    <div class="infosTop">
      <img :src="playlist.picUrl" v-if="Object.keys(playlist).length > 0"/>
    ...
    <div class="infosContent">
      <div class="infos">
        <div class="infosPic">
          ...
          <el-image class="img" :src="playlist.picUrl" :preview-src-list="[playlist.picUrl]" fit="cover" v-if="Object.keys(playlist).length > 0"></el-image>
          <img src="" alt="暂无图片" v-else/>
        </div>
        <div class="infosText">
          ...
          <div class="infosAuthor">
            <img :src="playlist.artist.img1v1Url" v-if="Object.keys(playlist).length > 0">
            <img src="" alt="暂无图片" v-else>
            <span v-if="Object.keys(playlist).length > 0">{{playlist.artist.name}}</span>
      ...
      <div class="operation">
        <div class="Collection">
          ...
          <span v-if="Object.keys(playlist).length > 0">{{changeCount(playlist.info.likedCount)}}</span>
          <span v-else>收藏数</span>
        </div>
        <el-divider direction="vertical" />
        <div class="comment">
          <el-icon><ChatLineRound /></el-icon>
          <span v-if="Object.keys(playlist).length > 0">{{changeCount(playlist.info.commentCount)}}</span>
          <span v-else>评论数</span>
        </div>
        <el-divider direction="vertical" />
        <div class="share">
          <el-icon><Share /></el-icon>
          <span v-if="Object.keys(playlist).length > 0">{{changeCount(playlist.info.shareCount)}}</span>
...
import {Star,Download,VideoCamera,Back,Search,StarFilled,MoreFilled,ArrowRight,ArrowDownBold,VideoPlay,CirclePlus,ChatLineRound,Share} from '@element-plus/icons-vue'
