<template>
  ...
  <div v-if="Object.keys(playlist).length > 0">
    <div class="blur"></div>
    <div class="infosTop">
      <img :src="playlist.coverImgUrl" v-if="musicPicUrl===''"/>
      <img :src="musicPicUrl" v-else/>
      <div class="left">
        <!--      返回上一级路由-->
      ...
      <div class="right">
        ...
        <el-icon class="Edit" @click="editMusicListButton"><Edit /></el-icon>
      </div>
      <!--    列表搜索搜索-->
            ...
            <el-form :model="form2"
              ...
              <el-form-item prop="name">
                <input
                    ...
                    name="name"
                    class="name"
                    placeholder="请输入歌单名称" v-model="form2.name">
              </el-form-item>
            </el-form>
          </div>
          <div class="btnArea">
    ...
    <div class="infosContent" id="infosContent">
      <div class="infos">
        <div class="infosPic">
          ...
          <el-image class="img" :src="playlist.coverImgUrl" :preview-src-list="[playlist.coverImgUrl]" fit="cover" v-if="musicPicUrl===''"></el-image>
          <el-image class="img" :src="musicPicUrl" :preview-src-list="[musicPicUrl]" fit="cover" v-else></el-image>
        </div>
        <div class="infosText">
          <div class="infosTitle">
            <span v-if="musicListName===''">{{playlist.name}}</span>
            <span v-else>{{musicListName}}</span>
          </div>
          ...
          <div class="infosIntroduce">
            <span v-if="musicListDesc===''">{{playlist.description}}</span>
            <span v-else>{{ musicListDesc }}</span>
            <i>...</i>
          </div>
        </div>
      </div>
    ...
    <div class="editArea">
      <el-drawer v-model="editShow"
                 ref="editShowDrawer"
                 title="标题"
                 :with-header="false"
                 :direction="direction" size="40%">
        <div class="editArea_layout">
          <el-form :model="form"
                   size="large"
                   :rules="rules"
            ...
            <el-form-item prop="edit_name">
              <input type="text"
                     name="edit_name"
                     class="edit_name"
                     placeholder="歌单名称" v-model="form.edit_name">
            </el-form-item>
            <el-form-item prop="edit_describe">
              <input type="text"
                     name="edit_describe"
                     class="edit_describe"
                     placeholder="歌单描述" v-model="form.edit_describe">
            </el-form-item>
          </el-form>
          <div class="btnArea">
            <button class="btn" @click="editMusicList">提交</button>
          </div>
        </div>
      </el-drawer>
    </div>
...
import {Star,Download,StarFilled,VideoCamera,Back,Search,MoreFilled,ArrowRight,ArrowDownBold,VideoPlay,CirclePlus,ChatLineRound,Share,Edit} from '@element-plus/icons-vue'
import {downloadMusic, getMv} from "../../request/api/musicList.js";
import {
  addMusicToList, createMusicList,
  getMusicListSongAll,
  uploadListDesc,
  uploadListName,
  uploadListPic
} from '../../request/api/userMusicList.js'
...
const searchAllMusic = async () => {
  ...
  if (isLoadingAllMusic.value === 0){
    ...
    let res = await getMusicListSongAll(props.id,store.state.Cookies)
...
//修改表单信息
const editMusicList = async function()  {
  await formRef.value.validate( async (valid) => {
    if (valid){
      const loadingInstance = ElLoading.service({
        target:document.getElementById('infosContent') ,
        text:"更新中",
        background:"rgba(52,52,52,0.8)",
      })
      if (picFile.value.name !== undefined){
        await uploadImg(picFile.value)  //上传歌单封面
      }else{
        console.log("未修改封面")
      }
      //修改歌单名称
      let res = await uploadListName(props.id,form.value.edit_name,store.state.Cookies)
      if(res.data.code===200){
        musicListName.value = form.value.edit_name
      }
      //修改歌单描述
      let res2 = await uploadListDesc(props.id,form.value.edit_describe,store.state.Cookies)
      if(res2.data.code===200){
        musicListDesc.value = form.value.edit_describe
        loadingInstance.close()
        editShowDrawer.value.handleClose()
      }
...
//上传歌单封面
const uploadImg = async function (file) {
  let formData = new FormData()
  formData.append('imgFile',file)
  let imgSize = await getImgSize(file)
  let res = await uploadListPic(formData,props.id,imgSize.width,0,0,store.state.Cookies)
  // console.log(res)
  musicPicUrl.value = res.data.data.url
}
//获取上传图片尺寸
const getImgSize = async function(file) {
...
const createAddMusicList= async function (){
  await formRef2.value.validate(async (valid) => {
    ...
    if (valid){
      ...
      let res = await createMusicList(form2.value.name,store.state.Cookies)
      console.log(res)
...
<script>
import $ from "jquery";
import {uploadListDesc, uploadListName, uploadListPic} from "../../request/api/userMusicList.js";
...
export default {
  ...
  methods:{
    ...
    //         background:"rgba(52,52,52,0.8)",
    //       })
    //       if (this.picFile.name !== undefined){
    //         await this.uploadImg(this.picFile)  //上传歌单封面
    //       }else{
    //         console.log("未修改封面")
    //       }
    //       //修改歌单名称
    //       let res = await uploadListName(this.props.id,this.form.edit_name,store.state.Cookies)
    //       if(res.data.code===200){
    //         this.musicListName = this.form.edit_name
    //       }
    //       //修改歌单描述
    //       let res2 = await uploadListDesc(this.props.id,this.form.edit_describe,store.state.Cookies)
    //       if(res2.data.code===200){
    //         this.musicListDesc = this.form.edit_describe
    //         loadingInstance.close()
    //         this.$refs.editShowDrawer.handleClose()
    //       }
    //
    //     }else {
    //       console.log("不合法输入")
    ...
    //   let formData = new FormData()
    //   formData.append('imgFile',file)
    //   let imgSize = await this.getImgSize(file)
    //   let res = await uploadListPic(formData,this.props.id,imgSize.width,0,0,store.state.Cookies)
    //   // console.log(res)
    //   this.musicPicUrl = res.data.data.url
    // },
    // //获取上传图片尺寸
    // getImgSize :async function(file) {
    //   return new Promise((resolve, reject) => {
    //     let reader = new FileReader()
    //     reader.readAsDataURL(file)
    //     reader.onload = function (theFile) {
